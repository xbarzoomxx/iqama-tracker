          ...
          const calcRecord = r => {
            ...
            if (isEmployee) {
              ...
              if (calcBacklogOnly) {
                ...
                // ── وضع يدوي: تجديد بالمدة المختارة ──
                renewPassport = quarters * (isExpired ? rateQ : PASSPORT_NORMAL_PER_Q);
                renewWork     = quarters * WORK_PERMIT_PER_Q;
                if (isWithinQuarter) {
                  currentPassport = Math.ceil(currentPartialCost * PASSPORT_NORMAL_PER_Q);
                  currentWork     = Math.ceil(currentPartialCost * WORK_PERMIT_PER_Q);
                }
              }
              ...
              if (calcBacklogOnly) {
                if (!isExpired) {
                  // سارية أو قريبة → تجديد ربع واحد فقط
                  renewPassport = 1 * PASSPORT_DEP_PER_Q;
                }
              } else {
                if (isWithinQuarter) {
                  currentPassport = Math.ceil(currentPartialCost * PASSPORT_DEP_PER_Q);
                }
            ...
            <div>
              ...
              <div style={{background:darkMode?"#1c1f26":"#fff",borderRadius:12,padding:"16px 20px",marginBottom:14,boxShadow:darkMode?"0 2px 8px rgba(0,0,0,0.5)":"0 2px 8px rgba(0,0,0,0.07)",display:"flex",gap:16,flexWrap:"wrap",alignItems:"flex-start"}}>
                ...
                <div style={{marginRight:"auto",background:darkMode?"#0a2218":"#f0fdf4",border:`1px solid ${darkMode?"#166534":"#86efac"}`,borderRadius:10,padding:"10px 18px",textAlign:"center",minWidth:190}}>
                  <div style={{fontSize:11,color:darkMode?"#4ade80":"#15803d",fontWeight:600}}>{calcBacklogOnly?"🔄 حساب تلقائي":`إجمالي التكلفة · ${calcMonths} شهر`}</div>
              ...
              <div style={{background:darkMode?"#1c1f26":"#fff",borderRadius:12,boxShadow:darkMode?"0 2px 8px rgba(0,0,0,0.5)":"0 2px 8px rgba(0,0,0,0.07)",overflow:"hidden"}}>
                ...
                {breakdown.sort((a,b)=>priorityOrder(a.r)-priorityOrder(b.r)||getDaysLeft(a.r.expiryDate)-getDaysLeft(b.r.expiryDate)).map(({r,isEmployee,isExpired,isWithinQuarter,expiredDays,useLateRate,totalBacklog,totalCurrent,totalRenew,total,expiredQuarterDisplay},i)=>{
                  ...
                  return (
                    <div key={r.id} style={{padding:"9px 16px",borderBottom:`1px solid ${darkMode?"#2e3340":"#f3f4f6"}`,background:i%2===0?(darkMode?'#1c1f26':'#fff'):(darkMode?'#191c23':'#fafafa'),display:"grid",gridTemplateColumns:"2fr 0.7fr 0.8fr 0.9fr 0.9fr 0.9fr 1fr",gap:6,alignItems:"center"}}>
                      <div>
