...
const DB_PATH = "iqama_records";

const STORAGE_KEY = "iqama_tracker_v3"; // احتياطي محلي فقط
...
function getStatus(r) {
  ...
  if (days <= 90) return "تنتهي قريباً";
  return "سارية";
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(dbRef, (snapshot) => {
      ...
      if (data && Array.isArray(data) && data.length > 0) {
        ...
        // أول مرة: ارفع البيانات الأولية
        set(ref(db, DB_PATH), INITIAL_DATA);
      ...
      try { const s=localStorage.getItem(STORAGE_KEY); if(s){setRecords(JSON.parse(s));} else {setRecords(INITIAL_DATA);} } catch{ setRecords(INITIAL_DATA); }
      setDbLoading(false);
    });
    return () => unsubscribe();
  }, []);

  // حفظ في Firebase عند التغيير (فقط إذا التغيير من المستخدم لا من Firebase)
  useEffect(() => {
    if (dbLoading) return;
    if (isRemoteUpdate.current) { isRemoteUpdate.current = false; return; }
    if (records.length > 0) {
      set(ref(db, DB_PATH), records);
      localStorage.setItem(STORAGE_KEY, JSON.stringify(records)); // نسخة احتياطية
    }
  }, [records]);
  ...
  if (dbLoading) return (
    <div style={{minHeight:"100vh",background:darkMode?"#111318":"#f0f4f8",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Segoe UI',Tahoma,sans-serif",direction:"rtl"}}>
      <div style={{textAlign:"center"}}>
        <div style={{fontSize:48,marginBottom:16}}>🔄</div>
        <div style={{fontSize:18,fontWeight:700,color:darkMode?"#e8eaf0":"#1e3a5f"}}>جارٍ تحميل البيانات...</div>
        <div style={{fontSize:13,color:darkMode?"#9299aa":"#6b7280",marginTop:8}}>جارٍ الاتصال بقاعدة البيانات</div>
      </div>
    </div>
  );
      ...
      <div style={{maxWidth:1200,margin:"0 auto",padding:"18px 14px",color:darkMode?"#e8eaf0":"inherit"}} onClick={()=>setShowExportMenu(false)}>
        ...
        {activeTab==="alerts"&&(
          <div style={{background:darkMode?"#1c1f26":"#fff",borderRadius:14,padding:22,boxShadow:darkMode?"0 2px 10px rgba(0,0,0,0.5)":"0 2px 10px rgba(0,0,0,0.07)"}}>
            ...
            {records.filter(r=>r.expiryDate&&getDaysLeft(r.expiryDate)<=90).length===0?(
              <div style={{textAlign:"center",color:"#6b7280",padding:30}}><div style={{fontSize:44}}>✅</div><p>لا توجد تنبيهات.</p></div>
            ):records.filter(r=>r.expiryDate&&getDaysLeft(r.expiryDate)<=90).sort((a,b)=>getDaysLeft(a.expiryDate)-getDaysLeft(b.expiryDate)).map(r=>{
        ...
        {activeTab==="cost"&&(()=>{
          ...
          const countSoon    = allEmployees.filter(e => getStatus(e) === "تنتهي قريباً").length;
